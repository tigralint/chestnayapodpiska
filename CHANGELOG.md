...
### 📚 Новые руководства и оптимизация радара
- **Актуализация базы знаний**: Добавлены инструкции по возврату средств для FitStars, MyBook, Академии Eduson, XYZ School и Зерокодер. Соответствующие правки внесены в системный промпт ИИ-ассистента.
- **Оптимизация «Народного радара» на GPU**: Полностью убрана нагрузка на GPU за счет исключения скругления углов у вращающегося луча (вращается как плоская квадратная текстура в статичной маске). Добавлена принудительная композиция слоев (`will-change`) для пульсирующих волн.
- **Пиксельное выравнивание**: Все элементы радара переведены на четную сетку соосности, устранены погрешности смещения на 0.5px. Центральная иконка перенесена внутрь RadarCanvas для идеальной соосности.
- **Чистка проекта**: Удален неиспользуемый мертвый код (типы и экспорты), исправлены уязвимости зависимостей (`npm audit fix`). Настроена автоматическая сортировка классов Tailwind CSS в Prettier и отключено форматирование `.md` файлов.
...
- Тесты: 360/360 passed, 44 файла.
- Vite build: success (392 modules).

### 🎨 UI/UX Best Practices & Performance
- **Анимации**: Длительность основных анимаций (`fade-in`, `slide-in-right`, `slide-up`) сокращена с 800ms до 400-500ms для повышения отзывчивости интерфейса без потери плавности.
- **Типографика**: Ограничена максимальная ширина текстовых блоков на страницах FAQ, Privacy и Terms (`max-w-4xl` → `max-w-3xl`) для достижения оптимальной длины строки при чтении.
- **Иконки**: Системные эмодзи заменены на консистентные SVG-иконки (Folder, Pencil, Rocket) из набора Lucide.
- **Интерактивность**: Увеличены зоны тапа кнопок-фильтров в Радаре, добавлены явные стили фокуса `focus-visible:ring-2` и семантичные текстовые `<label>`.
- **Контрастность**: Повышена контрастность ссылок в футере приложения (`text-white/40` → `text-white/60`).
