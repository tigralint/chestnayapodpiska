...
### 🛠️ Исправления ошибок, безопасность и рефакторинг
- **Исправление бесконечного рендера**: Устранена проблема с лишними ре-рендерами в `SearchInput` за счет сохранения стабильной ссылки `onChange` в `useRef`.
- **Gemini API & RadarService**: Исправлен порядок обработки HTTP-ответов (проверка `response.ok` перенесена перед чтением `.json()`), предотвращая маскировку ошибок парсинга.
- **Безопасность чата**: Внедрена санитизация сообщений `sanitizeForChat` в `assistant.ts`, предотвращающая Prompt Injection в диалогах с ИИ.
- **Оптимизация localStorage**: В хук `useChatHistory` добавлен дебаунс на 500мс для снижения дисковой нагрузки I/O при частых обновлениях в процессе стриминга сообщений.
- **Валидация**: Добавлена проверка на пустое поле `amount` (минимальное значение `.min(1)`) в Zod-схему для генерации претензии.
- **Устранение дублирования (DRY)**: Создан единый переиспользуемый компонент `ConsentCheckbox` для чекбокса согласия 152-ФЗ, заменивший дублирующийся код в трёх вьюхах.
- **Оптимизация хуков**: Функции `handleGenerate` (`useClaimForm`) и `saveToLocalStorage` (`useClaimHistory`) мемоизированы и вынесены для уменьшения лишних перерисовок.
- **Тесты и форматирование**: Исправлен упавший из-за фейковых таймеров тест `useChatHistory.test.ts`, кодовая база полностью отформатирована с помощью Prettier.
